  ...
  <div class="space-y-4">
    ...
    <div class="flex flex-col sm:flex-row justify-between items-center gap-4">
      <div class="flex items-center space-x-3 self-start">
        <button @click="toggleSelectAll" class="flex items-center px-3 py-1.5 text-sm rounded-md border"
          ...
          <svg v-if="allSelected" xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" viewBox="0 0 20 20"
            fill="currentColor">
          ...
          <svg v-else xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1.5" fill="none" viewBox="0 0 24 24"
            stroke="currentColor">
    ...
    <div class="flex flex-wrap items-center space-x-4 text-sm">
      ...
      <div class="ml-auto text-gray-500">
        {{ filteredImages.length }} of {{ uploadStore.images.length }} images
      </div>
    </div>

    <!-- Image Grid -->
    <div v-if="filteredImages.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
      <div v-for="image in filteredImages" :key="image.id"
        ...
        :class="{ 'ring-2 ring-brand-purple': isSelected(image.id) }">
        <!-- Checkbox overlay -->
        <div class="absolute top-2 left-2 z-10 cursor-pointer" @click.stop="toggleImageSelection(image.id)">
          <div class="h-5 w-5 flex items-center justify-center rounded border"
            :class="isChecked(image.id) ? 'bg-brand-purple border-brand-purple' : 'bg-white border-gray-300'">
            <svg v-if="isChecked(image.id)" xmlns="http://www.w3.org/2000/svg" class="h-3 w-3 text-white"
        ...
        <div class="absolute top-2 right-2 z-10 cursor-pointer h-5 w-5 rounded-full flex items-center justify-center"
          :class="image.isAnnotated ? 'bg-green-500' : 'bg-gray-300'" @click.stop="toggleAnnotation(image.id)"
        ...
        <div class="p-2 text-sm">
          ...
          <p class="text-xs text-gray-500 truncate">{{ formatDate(image.uploadedAt) }}</p>
        </div>
    ...
    <!-- Empty state -->
    <div v-else-if="uploadStore.images.length === 0"
    ...
    <modal v-model="showPreview" size="lg">
      <template #header>
        <h3 class="text-lg font-medium text-gray-900 truncate" :title="previewImage?.name">
          {{ previewImage?.name }}
        </h3>
      </template>
      ...
      <div v-if="previewImage" class="flex flex-col md:flex-row gap-6">
        ...
        <div class="md:w-2/3 flex items-center justify-center bg-gray-100 rounded-lg overflow-hidden">
          <img :src="previewImage.url" :alt="previewImage.name" class="max-h-[60vh] object-contain" />
        </div>
        ...
        <div class="md:w-1/3 space-y-4">
          <div>
            ...
            <div class="grid grid-cols-[auto,1fr] gap-x-3 gap-y-1 text-sm">
              ...
              <button @click="toggleAnnotation(previewImage.id)" class="flex items-center text-sm text-left"
                :class="previewImage.isAnnotated ? 'text-green-600' : 'text-gray-500'">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 mr-1"
                  :class="previewImage.isAnnotated ? 'text-green-600' : 'text-gray-400'" fill="none" viewBox="0 0 24 24"
                  stroke="currentColor">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                </svg>
                {{ previewImage.isAnnotated ? 'Annotated' : 'Not Annotated' }}
              </button>
            </div>
          </div>
      ...
      <template #footer>
        <div class="flex justify-between">
          <button @click="confirmDeleteImage(previewImage?.id || '')"
    ...
    <modal v-model="showDeleteConfirm" title="Confirm Deletion">
      <p class="text-gray-600 mb-4">Are you sure you want to delete
        {{ deleteTarget === 'selected' ? selectedCount + 'selected' : 'this' }} image(s)? This action cannot be undone.
      </p>
...
const filteredImages = computed(() => {
  return uploadStore.images.filter(img => {
    // Text search
    const matchesSearch = !searchQuery.value ||
      img.name.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
      img.batchName.toLowerCase().includes(searchQuery.value.toLowerCase()) ||
...
const annotatedCount = computed(() =>
  uploadStore.images.filter(img => img.isAnnotated).length
);
...
const selectedCount = computed(() =>
  uploadStore.selectedImageIds.length
);

const allSelected = computed(() =>
  uploadStore.selectedImageIds.length === uploadStore.images.length &&
  uploadStore.images.length > 0
);

const someSelected = computed(() =>
  uploadStore.selectedImageIds.length > 0 &&
  !allSelected.value
);
...
const toggleSelectAll = () => {
  if (allSelected.value) {
    uploadStore.deselectAllImages();
  } else {
    uploadStore.selectAllImages();
  }
};

const isSelected = (id: string) => {
  return uploadStore.selectedImage?.id === id;
};

const isChecked = (id: string) => {
  return uploadStore.selectedImageIds.includes(id);
};

const toggleImageSelection = (id: string) => {
  uploadStore.toggleImageSelection(id);
};

const toggleAnnotation = (id: string) => {
  uploadStore.toggleAnnotationStatus(id);
};

const openImagePreview = (image: ImageFile) => {
...
const addTagToImage = () => {
  if (previewImage.value && newImageTag.value.trim()) {
    ...
    if (!previewImage.value.tags.includes(tag)) {
      uploadStore.updateImageTags(
        previewImage.value.id,
        [...previewImage.value.tags, tag]
      );
      uploadStore.addTag(tag);
    }
    newImageTag.value = '';
  }
};
...
const removeTagFromImage = (imageId: string, tag: string) => {
  const image = uploadStore.images.find(img => img.id === imageId);
  if (image) {
    uploadStore.updateImageTags(
      imageId,
...
const confirmDeleteSelected = () => {
  ...
  if (previewImage.value && uploadStore.selectedImageIds.includes(previewImage.value.id)) {
    showPreview.value = false;
  }
};
...
const executeDelete = () => {
  if (deleteTarget.value === 'selected') {
    uploadStore.deleteSelectedImages();
  } else if (imageToDelete.value) {
    uploadStore.deleteImage(imageToDelete.value);
  }

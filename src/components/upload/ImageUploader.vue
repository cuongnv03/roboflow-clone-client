    ...
    <div class="border-2 border-dashed rounded-lg p-8 text-center transition-colors"
      ...
      <div class="flex justify-center">
        <label class="btn btn-primary cursor-pointer">
          Browse Files
          <input type="file" class="hidden" accept="image/jpeg,image/png,image/webp,image/bmp,image/avif" multiple
            @change="onFileSelect" />
        </label>
      </div>
    </div>
    ...
    <div v-if="isUploading" class="bg-gray-50 rounded-lg border border-gray-200 p-4">
      <div class="flex justify-between items-center mb-2">
        <div class="flex items-center">
          <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-brand-purple" xmlns="http://www.w3.org/2000/svg" fill="none"
            viewBox="0 0 24 24">
          ...
          <span class="font-medium">Uploading {{ uploadingFiles }} files...</span>
        </div>
...
const handleFiles = async (files: File[]) => {
  ...
  try {
    ...
    uploadingFiles.value = validFiles.length;

    // Update upload store with batch name and selected tags
    uploadStore.setCurrentBatchName(batchName.value);
    selectedTags.value.forEach(tag => uploadStore.addTag(tag));
    ...
    const uploadPromises = validFiles.map((file, index) => {
      return new Promise<void>((resolve) => {
        ...
        const interval = setInterval(() => {
          ...
          const overallProgress = validFiles.map((_, i) =>
            i < index ? 100 : i === index ? progress : 0
          ).reduce((a, b) => a + b, 0) / validFiles.length;
    ...
    uploadStore.addFilesToQueue(validFiles, batchName.value, selectedTags.value);

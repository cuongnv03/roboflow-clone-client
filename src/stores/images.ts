...
import { ref } from 'vue';
// import imageService from '@/services/imageService'; // Import image service (khi cần tương tác API)
...
export const images = defineStore('image', () => {
  ...
  const addImages = async (
    ...
    if (newImages.length > 0) {
      ...
      // Có thể gọi imageService.uploadImages(newImages) ở đây nếu cần upload
    }
  };
  ...
  const deleteImage = (id: string) => {
    ...
    if (img) {
      ...
      // Có thể gọi imageService.deleteImageOnServer(id) ở đây nếu cần xóa trên server
    }
  ...
  const deleteSelectedImages = () => {
    selectedImageIds.value.forEach((id) => {
      ...
      // Có thể gọi imageService.deleteImageOnServer(id) cho từng ảnh được chọn
    });
  ...
  const updateImageTags = (id: string, tags: string[]) => {
    ...
    // Có thể gọi imageService.updateImageInfo(id, { tags }) nếu cần lưu lên server
  };

  const toggleAnnotationStatus = (id: string) => {
    const img = images.value.find((i) => i.id === id);
    if (img) img.isAnnotated = !img.isAnnotated;
    // Có thể gọi imageService.updateImageInfo(id, { isAnnotated }) nếu cần lưu lên server
  };
